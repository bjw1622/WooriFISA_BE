...
-- → 실행 결과 행의 수는 총 20개
select last_name, count(last_name) as 'Count of Last Name'from actor having count(last_name)>2 ;
...
-- → 실행 결과 행의 수는 총 2개
select first_name, last_name, address from staff as s join address a on s.address_id = a.address_id;
...
-- → 실행 결과 행의 수는 총 2개
select s.staff_id,s.first_name,s.last_name,sum(p.amount) from staff as s  join payment p on s.staff_id = p.staff_id where payment_date > '2005-08-01 00:00:00' and payment_date<'2005-08-02 00:00:00' group by s.staff_id;

-- 15. film 테이블의 별칭을 f, film_actor 테이블의 별칭을 fa로 짓고, 두 테이블을 연결(JOIN)하여 각 film_id가 일치하는 조건의 title 및 해당 film에 참여한 총 actor의 수를 의미하는 컬럼 ‘총 배우  fro수’ 컬럼을 film 테이블의 title 별로(grouping) 조회
...
-- → 실행 결과 행의 수는 599개
select c.first_name,c.last_name,sum(p.amount) from customer as c join payment p on c.customer_id = p.customer_id group by first_name,last_name order by last_name;